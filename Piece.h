...
public:
  ...
  /* The function getLegalTarget() returns a vector lists all legal squares
     that can be reached by a Piece from a specific positions.
  ...
  /*NOTE: Returning the vector by value is a conscious design decision, based on
    the assumption that any compiler adhering to c++11 has vector copy elision.
    ...
    the vector could become too large for the values it needs to store -> wasteful.*/
